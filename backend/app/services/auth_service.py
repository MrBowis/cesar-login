...
class AuthService:
    ...
    def setup_totp(self, user_id: UUID) -> Tuple[str, str]:
        ...
        # Cifrar el secret antes de guardarlo en la base de datos (AES-256)
        encrypted_secret = encrypt_secret(secret, settings.aes_encryption_key)
        
        # Guardar secret cifrado en la base de datos
        self.user_repository.update_totp_secret(user_id, encrypted_secret)
        
        # Generar URI para código QR (usa el secret en texto plano)
    ...
    def verify_totp_code(self, user_id: UUID, totp_code: str) -> bool:
        ...
        # Descifrar el secret de la base de datos (AES-256)
        try:
            decrypted_secret = decrypt_secret(user.totp_secret, settings.aes_encryption_key)
        except ValueError as e:
            raise ValueError(f"Error al descifrar el secret: {str(e)}")
        
        # Verificar código con el secret descifrado
        is_valid = self.totp_service.verify_totp(decrypted_secret, totp_code)
    ...
    def login(self, email: str, password: str, totp_code: Optional[str] = None) -> Tuple[Optional[str], Optional[User], str]:
        ...
        if not self.totp_service.verify_totp(decrypted_secret, totp_code):
